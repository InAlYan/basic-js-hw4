// Задание 1
// Необходимо с помощью цикла for вывести следующие 11 строк в консоль:
// 0 – это ноль
// 1 – нечетное число
// 2 – четное число
// 3 – нечетное число
// …
// 10 – четное число

console.log("Задание 1");
for (let i = 0; i < 11; i++) {
    if (i === 0)
        console.log(`${i} – это ноль`)
    else if (i % 2 === 0)
        console.log(`${i} – четное число`)
    else
        console.log(`${i} – нечетное число`);
}

// Задание 2
// Дан массив [1, 2, 3, 4, 5, 6, 7]
// Сделайте из этого массива следующий [1, 2, 3, 6, 7]

console.log("Задание 2");
const arrBegin = [1, 2, 3, 4, 5, 6, 7];
console.log(`Начальный массив ${arrBegin}`);
const arrEnd = [];
for (let i = 0; i < arrBegin.length; i++) {
    if (arrBegin[i] === 5) continue
    else arrEnd.push(arrBegin[i]);
}
console.log(`Конечный массив ${arrEnd}`);

// Задание 3
// Используя Math.random() вам необходимо генерировать цифры от 0 до 9, и создать массив состоящий из 5 таких элементов
// 1. Рассчитать сумму элементов этого массива
// 2. Найти минимальное число
// 3. Найти есть ли в этом массиве число 3

console.log("Задание 3");
const arr = [];
let sum = 0;
let minNumber = 9;
let isThree = false;
for (let i = 0; i < 5; i++) {
    arr[i] = Math.floor(Math.random() * 10);
    sum += arr[i];
    if (minNumber > arr[i]) minNumber = arr[i];
    if (arr[i] === 3) isThree = true;
}
console.log(`Сформированный массив ${arr}`);
console.log(`Сумма элементов массива ${sum}`);
console.log(`Минимальный элемент массива ${minNumber}`);
(isThree) ? console.log(`В массиве есть число 3`) : console.log(`В массиве нет числа 3`);

// *Необязательное задание. *
// Необходимо вывести горку в консоль (используя цикл for), как показано на рисунке, только у вашей горки должно быть 20 рядов, а не 5:

// x
// xx
// xxx
// xxxx
// xxxxx

console.log("Необязательное задание.");
const maxIter = 20;
for (let i = 1; i <= maxIter; i++) {
    s = "";
    for (let j = 1; j <= i; j++) {
        s += "x";
    }
    console.log(s);
}